#!/usr/bin/env python
r"""
FleetPro: Import AKUNGBA CR daily diesel readings.

Source: "AKUNGBA CR diesel usage daily report 2026.xlsx". Workbook holds many
unrelated trackers; diesel data is in monthly tabs JAN 26, FEB 26, MAR 26,
APRIL 26, MAY 26, JUNE 2026, ... (JULY..DECEMBER are empty future templates).
Location cell reads "AKUNGBA".

TWO COLUMN LAYOUTS in the same workbook:
  - JAN 26 / FEB 26: DATE in col 1 (no weekday column).
  - MAR 26 onward:   a DAY-OF-WEEK column was inserted at col 1, so DATE moved
                     to col 2 and everything shifts right by one.
We DETECT the date column per tab (the col whose data rows are datetimes) and
anchor every other column off it: base = date_col - 1. Verified across all
months (opening - closing + purchases = consumption; close - open = hours run).
  ...
  base+15 GENERATOR CONSUMPTION (LITRES/HOUR)   (sheet ~5-6 L/hr; we recompute)
  base+16 NEPA METER Opening
  base+17 NEPA Closing

WHY THIS IMPORT MATTERS: Akungba had no baseline, so the app fell back to a
default 15 L/hr and falsely flagged honest readings. The sheet proves the gen
runs ~5-6 L/hr all year. Run with --replace to overwrite the 3 unreliable
first-time app entries (Jun 17-19, incl. a bad 227 L Jun-17 row), then
--recalc-baseline so the true ~5.7 L/hr rate is learned.

Usage:
  py scripts\import_akungba_cr.py                              # dry run
  py scripts\import_akungba_cr.py --apply --replace --recalc-baseline
...
FILE = Path(r"C:\Users\MakindeAribo\Downloads\AKUNGBA CR diesel usage daily report 2026.xlsx")
STORE_NAME = "Akungba CR"
GEN_LOC_MATCH = "akungba"
YEAR = 2026

MONTH_TABS = {
    "JAN 26": 1, "FEB 26": 2, "MAR 26": 3, "APRIL 26": 4, "MAY 26": 5, "JUNE 2026": 6,
    "JULY 2026": 7, "AUGUST 2026": 8, "SEPTEMBER 2026": 9, "OCTOBER 2026": 10,
    "NOVEMBER 2026": 11, "DECEMBER 2026": 12,
}
    ...
    for tab, month in MONTH_TABS.items():
        ...
        for r in range(1, ws.max_row + 1):
            ...
            out.append({
                ...
                "gen_h_open": gh_open, "gen_h_close": gh_close, "closing": closing,
                "purchases_in": n(ws.cell(r, col("purchases_in")).value),
                "consumption_l": cons, "consumption_rate": rate,
...
def main(apply_mode, recalc_mode, replace_mode):
    ...
    gen = next((g for g in gens if (g.get("loc") or "").strip().lower() == "akungba cr"
    ...
    if not gen:
        ...
        sys.exit(1)
    gen_id = gen["id"]
    print(f"  Akungba generator: {gen['name']} (id={gen_id}, loc={gen.get('loc')})")
    ...
    if not apply_mode:
        print("\nDRY RUN — no writes. Re-run with --apply (add --replace --recalc-baseline).")
        if recalc_mode:
            print("\n=== BASELINE RECALC (Akungba) ===")
    ...
    if new_readings:
        ...
        print(f"  + Inserted {total} Akungba readings")
    print("\nDONE.")
    if recalc_mode:
        print("\n=== BASELINE RECALC (Akungba) ===")
        recalc_baseline(sb, gen_id)
