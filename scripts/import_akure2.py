#!/usr/bin/env python
r"""
FleetPro: Import AKURE 2 CR daily diesel readings.

Source: "DIESEL REPORT 2026.xlsx" — ONE SHEET PER MONTH (JAN..JUNE 2026),
location cell reads "AKURE 2 CR" -> store "Akure 2".

COLUMN LAYOUT (verified: opening - closing + purchases = consumption;
close - open = hours run). This template has an explicit HOUR RUN column (15),
which pushes the generator rate to 16 and NEPA to 17/18 (one further right than
the OWO CR template). The closing tank level lives in the TOTAL CLOSING column
(9); the "Closing Stock - Main" column (8) is left blank in this file.

  1  DATE
  2  SUPPLIER (blank in this file even on purchase rows)
  ...
  7  DIESEL AVAILABLE
  8  CLOSING STOCK - MAIN      (blank here)
  9  TOTAL CLOSING STOCK       -> diesel_level_actual
  10 CONSUMPTION IN LITRES
  11 DIESEL COST (NAIRA)
  12 GENERATOR RUNNING HOUR Opening
  13 GENERATOR RUNNING HOUR Closing
  15 HOUR RUN (HOURS)
  16 GENERATOR CONSUMPTION (LITRES/HOUR)   (sheet value unreliable; we recompute)
...
Usage:
  py scripts\import_akure2.py                       # dry run
  py scripts\import_akure2.py --apply               # write readings
  py scripts\import_akure2.py --apply --recalc-baseline
...
FILE = Path(r"C:\Users\MakindeAribo\Downloads\DIESEL REPORT 2026.xlsx")
STORE_NAME = "Akure 2"          # must match generators.loc / locations.name
GEN_LOC_MATCH = "akure 2"        # case-insensitive loc match to find the generator
...
COL = {
    ...
    "closing_main": 8,      # blank here; fallback for closing level
...
def parse_readings():
    ...
    for tab, (year, month) in MONTHLY_TABS.items():
        ...
        for r in range(1, ws.max_row + 1):
            ...
            # A real day has a closing tank level OR closing gen hours.
            ...
            # Recompute rate from level/hours (sheet col 16 is glitchy on some rows).
...
def main(apply_mode, recalc_mode, replace_mode):
    ...
    if not gen:
        ...
        sys.exit(1)
    gen_id = gen["id"]
    print(f"  Akure 2 generator: {gen['name']} (id={gen_id}, loc={gen.get('loc')})")
    ...
    if new_readings:
        ...
        print(f"  + Inserted {total} Akure 2 readings")
    print("\nDONE.")
    if recalc_mode:
        print("\n=== BASELINE RECALC (Akure 2) ===")
        recalc_baseline(sb, gen_id)
