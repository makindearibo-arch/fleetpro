...
3. **Import Ado 1/2/3 supply data — script ready (2026-06-11)**: `scripts/import_admin_supply.py` imports DIESEL REPORT TEMPLATE (3): bulk purchases (dedup by date+litres — survives supplier renames), STORES SUPPLY (full Jan 2025–Jun 2026 history, all name variants incl. ALAGBAKA*→Akure 1, Ado 1/2/3, bare OKITIPUPA→Okitipupa CR assumed), and the new OUTSOURCED SUPPLY tab (local suppliers Akinola/Sharfa/Kenny Betty, no price recorded → cost null, dists tagged "Outsourced:" in notes). Date cells repaired via chronology-aware day/month swap capped at 60-day jumps.
