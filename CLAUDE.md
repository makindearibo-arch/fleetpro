...
9. **Odometer readings in fuel_logs are NOT clean -- use `odoDistances()` (2026-09-19)**: staff type a repeated digit when they have no reading (`111111` x269, `1111` x99, `222222` x10), and several vehicles never update it (LSD80XA HYUNDAI TRUCK repeats the same reading on 237 of 241 fills). A naive `reading - previous reading` turns one placeholder into a -166,000 km row followed by a +167,000 km row. `odoDistances(logs)` (App.jsx, module level, above `DEFAULT_VEHICLE_GROUPS`) treats repdigit/zero readings as no reading, drops out-of-sequence typos by keeping the longest non-decreasing run of real readings (catches too-low AND too-high typos), and labels a distance "since <date>" when it spans fills with no usable reading. Used by the vehicle detail page's Distance column; verified against all 3,259 live vehicle fuel logs with zero negative results. **Still naive, not yet migrated**: `FuelPage.calcConsumption` (Fuel & Energy L/100km -- a placeholder gives ~0.04 L/100km on the next fill), and `ReportsPage.renderFuel`, which reads `f.odoEnd`/`f.odoStart` -- fields `toFL` never sets -- so the Fuel Consumption report's distance is always 0 km.
