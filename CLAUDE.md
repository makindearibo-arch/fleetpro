...
4. **Roll out import to remaining stores** (Akure 3/4/5, Idanre, Igbokoda, Ikare Bakery, Okitipupa Bakery, Ondo Bakery/CR/Ondo 2 CR, Owo Bakery, Oye Bakery, Pie Express/Warehouse). Done so far: Akure 1, Akure 2, Akure 6, Ado 1, Owo CR, Okitipupa CR, Ikare CR, Akungba CR, all 7 bakeries.
   ...
   - **recalc_baselines.py is now gap-aware (2026-06-24)**: only counts a level-delta rate when the previous reading is the calendar day before. A multi-day logging gap (e.g. Akungba's March 25→April 1) otherwise lumps several days' consumption against one day's hours → a ~50 L/hr phantom that poisons the mean (Akungba 7.03→6.77, max 50.21→15.46). Benefits every generator with gaps. Re-run order unchanged: recalc_baselines --apply → backfill_discrepancy_flags --apply.
...
0. **"Store staff see 0 readings" — TWO root causes, both FIXED 2026-06-24:**
   - **(a) Auth-lock contention** (`src/supabase.js`): supabase-js guards auth-token access with the browser Web Locks API (`navigator.locks`), which is shared ACROSS tabs. The app's ~23 concurrent initial fetches (and/or a second open FleetPro tab) contended for it and supabase aborted the losers with `AbortError: Lock broken by another request with the 'steal' option` — those fetches returned empty (console showed `Data loaded {... dr:0}`). FIX: the client now uses a custom **in-memory, per-tab lock** (promise chain per name) instead of navigator.locks. If this error ever returns, check `src/supabase.js` `inMemoryLock`.
