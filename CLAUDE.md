...
2. **Vehicle diesel transfer feature — BUILT (2026-06-10)**. `diesel_transfers` table (migration `20260610_diesel_transfers.sql`: date, store, source_generator_id, dest_type vehicle/oven/other, dest_id, dest_label, litres). Staff UI: Diesel Log → "Transfer Diesel" tab (date-locked like readings; vehicle picker with free-text fallback, oven picker at bakeries). Admin: Diesel Management → "transfers" tab. Save-time discrepancy + baseline learning subtract same-day transfers; Watchtower overnight-gap math credits transfers. Scripts: `backfill_transfers_from_notes.py` (parses "Transfer-out:" notes into transfer rows — vehicle "NAME (litres)" pairs matched to vehicles by plate/name, bare numbers at bakeries → oven), `recalc_baselines.py` (transfer-aware, ALL generators, replaces per-import recalcs), `backfill_discrepancy_flags.py` (now transfer-aware). Run order after any transfer data change: backfill_transfers → recalc_baselines --apply → backfill_discrepancy_flags --apply. **ORPHANED-TRANSFER FIX (2026-07-14)**: staff-entered transfers were saved with `source_generator_id=null` (the form's "From (source tank)" defaulted to "Store tank / unspecified"), so consumption/discrepancy/baseline/Watchtower — all of which match transfers by `source_generator_id` — silently ignored them and the reading falsely flagged the transferred litres as a loss (Akure 1 Jul 13–16 all flagged). Fixes: (a) TransferSection now auto-selects the store's tank as source (single non-oven gen, or the oven at oven-only stores) and the field is required; (b) DieselLogPage handleSave credits transfers matched by `sourceGenId===selGen` OR (`null` + same store) as a fallback; (c) data-fixed the 17 existing null-source rows to their store's primary asset. After re-running recalc+flags, Akure 1 Jul 16 (100 L transferred = 100 L drop) is unflagged; Jul 13–15 STILL flag because the logged transfers + gen burn only explain part of the tank drop (~95–120 L/day unaccounted while the gen ran <1 h) — a REAL "transfers under-logged or diesel unaccounted" signal, not a code bug. (e) **Readings "Expected" column = hours × baseline (the model), NOT `consumption_litres` (2026-07-15)**: for imported rows `consumption_litres` holds the SHEET's actual consumption (= the transfer-aware "Used"), so the old Expected column equalled Used yet the row flagged against a hidden third number (hours×baseline). Now `expectedOf = hoursRun × baseline_rate` (from genBaselines) so **Used − Expected = the gap = |stored discrepancy| = the flag** — fully consistent. The flag modal likewise breaks the tank drop into Transferred-out vs Used-by-generator and computes expected as hours×rate (verified: Akure 1 Jun 29 used 289 vs expected 100 = gap 189; Jul 15 used 120 vs expected 87 = gap 33). (d) The Diesel Management **Readings table now has a "Transferred Out (L)" column** (per gen+date, purple) + a "Transferred Out" total KPI, the destinations appended to the Notes cell ("Transferred: 50 L → PLATE …") and tooltip, and both carried into the CSV export — so a transfer day is self-explanatory (Jul 16: Used ~0, Transferred Out 100 L to two vehicles).
...
3b. **DONE — Ado 1 (2026-07-02) + Akure 6 (2026-07-14).** Ado 1: see its bullet under item 4 (old phantom Apr-23 row deleted first). **Akure 6 REFRESHED**: 508 readings Feb 20 2025–Jul 12 2026 from `AK6 Daily Diesel Tracker New (1).xlsx` via `scripts/import_akure6.py` (`--replace` deletes ALL 458 old rows, incl. pre-Feb-20 Cowork-era ones). Narrow layout (like Ado 1); quirky tab names (`OCT. 2025`, `JAN2026`, `MAY,2026`), and the "MARCH 2025" tab starts on Feb 20 — so the importer reads every dated row and dedups by REAL date, not tab name. **NO transfer-out data** in col 6 (Akure 6 doesn't transfer to vehicles — so its high flag rate was never transfers, it's meter discipline). Meter is unreliable: some cells hold a DATE instead of an hour value (e.g. `1934-11-16`); `n()` rejects non-numeric so those 11 rows import with level data but no hours. consumed 40,721 L vs added 37,853 (coherent). Pooled baseline 7.32 L/hr + gap floor: **flags 49%→24%**. Hours synced to 17,979.
...
8. **Reports date filter ignored undated rows (FIXED 2026-09-10)**: `ReportsPage.inRange` was `d=>{if(!d)return true;...}` -- a row with no date passed EVERY date window. `work_orders.due` is a **TEXT** column and `fromWO` writes `due: w.due||""`, so a work order without a due date stores an **empty string, not NULL** (`due=is.null` counts 0; the blanks only show up as falsy in JS). 433 of 614 work orders have a blank due (432 imported Completed + 1 Open), so the Maintenance & WO tab showed the identical 432 Completed / N33.3M cost / full unfiltered table for every range, plus a phantom extra "Open". Fixed: `inRange` returns false for a falsy date, and the tab shows an amber banner counting the excluded undated work orders instead of dropping them silently. Do NOT "fix" this by falling back to `created_at` -- all 432 imported rows share `created_at = 2026-02-28` (the bulk-import stamp), so that would lump the whole maintenance history onto one day. **BACK-FILLED 2026-09-10** by `scripts/backfill_wo_due_dates.py` from the Fleetio export "service_entries (1).csv" (433 rows with a real `Completed At`, 2022-01-25..2026-02-18): 432/432 matched, 612 of 613 work orders now carry a due date (only the hand-entered WO-MOSSEEPB "test" row is left blank, deliberately). MATCHING GOTCHA: content matching only reaches 409/433 because the original import TRUNCATED `descr` to ~60 chars and re-joined multi-task entries with "|". The rows are instead aligned POSITIONALLY -- WO ids are sequential (WO-0001..WO-0433) and follow the CSV file order -- via a two-pointer walk keyed on (vehicle name, total cost) that tolerates skipped CSV rows. Exactly one CSV row was skipped by the original import (AKR891ZN MAN DIESEL TRUCK, N31,000, 04 Jul 2024 -- a vehicle not in the fleet), and from that row on a naive positional match is off by one, which is why the walk is needed. Description independently corroborates 412/432 pairs. Re-use this positional-walk pattern for any other Fleetio-sourced backfill. Also split `gWO` (group-scoped, undated) from `fWO` (group + date) so the Fleet Summary "Open Work Orders" KPI stays a live snapshot while honouring the vehicle-group filter.
9. **Meter readings in fuel_logs are NOT clean -- always go through `meterDeltas()` (2026-09-19)**: staff type a repeated digit when they have no reading (`111111` x269, `1111` x99, `222222` x10), 411 fills repeat the previous reading (meter not read -- mostly days apart, so NOT "0 km"), and several vehicles never update it (LSD80XA HYUNDAI TRUCK: no usable reading on any recent fill). A naive `reading - previous` turns one placeholder into a -166,000 km row then a +167,000 km row. Helpers (App.jsx, module level, above `DEFAULT_VEHICLE_GROUPS`): `isPlaceholderReading`, `meterDeltas(logs)` -> Map(id -> {delta, litres, since, flag}) where flag is "none" (placeholder/blank), "same" (not updated) or "seq" (out-of-sequence typo, rejected via the longest non-decreasing run of real readings, which catches too-low AND too-high typos); `litres` = fuel over the WHOLE span since the last usable reading, so km/L stays right when a distance covers several fills (APP95YD Mar 12: 923 km on 258 L = 3.6, not on 110 L = 8.4). `usualKmPerL` = median over measured fills (needs 5), `kmPerLColor` judges against the vehicle's OWN usual (red <60%, orange <80%) -- never a fleet-wide number, a King Long bus at 5 km/L is normal. `allMeterDeltas(fuelLogs)` computes over each asset's FULL history so a period report's first fill measures from the fill before the period. Works for generator hour meters too (delta = hours). Used by: vehicle detail Distance + km/L columns, Fuel & Energy km/L (memoised; was L/100km, O(n^2) and naive), Reports > Fuel Consumption (Distance/km/L/"Fills with reading" and generator Hours/L/hr -- previously read `odoEnd`/`odoStart`/`hrsEnd`/`hrsStart`, fields `toFL` never sets, so they were ALWAYS 0). Also fixed `FuelPage.getLastReading`, which compared `b.date` with itself (never sorted by date) and offered placeholders as the form's "Last reading" hint. CAVEAT: per-fill km/L is noisy for vehicles whose engine runs while parked -- APP95YD tipper, MUS58YH truck, FFN664XA tanker (usual 1.97 km/L, i.e. PTO pumping) land 31-39% of fills under 60% of usual, vs 5-7% for the Hiaces/buses. Red there is not by itself a theft signal.
10. **Attachments / Supabase Storage (2026-09-22)** -- one file per fuel log, work order and vehicle paper. Columns `fuel_logs.receipt_path`, `work_orders.invoice_path`, `papers.doc_path` hold the STORAGE PATH, not a URL: the `documents` bucket is PRIVATE and `openDoc()` mints a 1-hour signed URL on demand, because a stored public URL would expose an invoice or a vehicle registration to anyone who ever saw the link. Shared components in App.jsx next to `Field`: `DOC_BUCKET`, `openDoc`, `DocLink` (paperclip View link for list rows), `DocUpload` (form control: two pickers -- "Take photo" with capture="environment" for the phone camera, and "Choose file" without it for gallery/PDF/desktop -- plus Retake once attached; uploads immediately). Remove/Retake only DETACH the reference, they never delete the stored object: deleting on click meant removing then cancelling the form left a saved record pointing at a missing file. Unreferenced files accumulate in the private bucket -- a cleanup sweep is an open task. `shrinkImage()` compresses before upload (1600px longest edge, JPEG 0.8, only images over 400 KB; EXIF rotation applied via imageOrientation:"from-image", else portrait photos land sideways). Verified in-browser on the shipped function: a 4032x3024 photo goes 3,715 KB -> 606 KB, a receipt-like image 653 KB -> 129 KB and stays legible; PDFs, small images and undecodable HEIC all pass through untouched. The 10 MB cap is applied AFTER compression. **Migration `20260922_document_uploads.sql` must be run in the Supabase SQL editor** -- it adds the columns, creates the buckets and adds storage policies. **storage.objects keeps RLS ON with no policies by default**, unlike the public tables (20260512_disable_rls_all_public_tables), so a bucket alone is not enough: without a policy every browser upload is rejected. DEPLOY-SAFE PATTERN worth reusing (Vercel auto-deploys on push, so a new column cannot be assumed to exist): App scope probes each column with `select(col).limit(1)` and passes `docUploads.{fuel,wo,papers}` down; the pages hide the controls when false, and `fromFL`/`fromWO`/`fromP` include the `*_path` key ONLY when the caller passes a non-undefined value, so save payloads are unchanged while the feature is off. **BUG THIS UNCOVERED**: DieselLogPage has uploaded meter/NEPA/tank photos to a `meter-photos` bucket since it was written, but NO BUCKET EXISTED in the project at all -- `if(upErr||!upData)return""` swallowed every failure, so all 2,483 non-null `gen_photo_url` values are the empty string and no photo has ever been stored. The migration creates `meter-photos` (PUBLIC, since that code calls `getPublicUrl()`). Consequence to remember: Watchtower's "low photo rate last 30d" component (10 pts) has been penalising every store for a missing bucket, not staff behaviour. The diesel upload code still swallows errors silently -- worth surfacing them next time that file is touched.
