...
function SettingsPage({locations,setLocations,vendorTypes,setVendorTypes,users,setUsers,user,setUser}){
  ...
  return(<div style={{maxWidth:800}}><div style={{display:"flex",gap:8,marginBottom:20}}>{["profile","users","locations","vendor types"].map(t=>(<button key={t} onClick={()=>setTab(t)} style={{padding:"8px 20px",borderRadius:8,border:tab===t?`1.5px solid ${P}`:"1.5px solid #E0E0E0",background:tab===t?"#D0E2FF":"#fff",color:tab===t?P:"#525252",fontSize:13,fontWeight:600,cursor:"pointer",textTransform:"capitalize"}}>{t}</button>))}</div>
