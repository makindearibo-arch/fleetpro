...
const toFL=(r)=>({id:r.id,asset:r.asset,date:r.date,litres:Number(r.litres)||0,cost:Number(r.cost)||0,reading:Number(r.reading)||0,station:r.station,isGen:r.is_gen,fuelType:r.fuel_type,receiptPath:r.receipt_path||""});
const fromFL=(f)=>({asset:f.asset,date:f.date,litres:f.litres||0,cost:f.cost||0,reading:f.reading||0,station:f.station||"",is_gen:f.isGen||false,fuel_type:f.fuelType||"Diesel",...(f.receiptPath!==undefined?{receipt_path:f.receiptPath||""}:{})});
const toWO=(r)=>({id:r.id,asset:r.asset,type:r.type,priority:r.priority,status:r.status,desc:r.descr,assignee:r.assignee,due:r.due,cost:Number(r.cost)||0,isGen:r.is_gen,invoicePath:r.invoice_path||""});
const fromWO=(w)=>({id:w.id,asset:w.asset,type:w.type,priority:w.priority,status:w.status||"Open",descr:w.desc||"",assignee:w.assignee||"",due:w.due||"",cost:w.cost||0,is_gen:w.isGen||false,...(w.invoicePath!==undefined?{invoice_path:w.invoicePath||""}:{})});
const toP=(r)=>({id:r.id,vehicle:r.vehicle,docType:r.doc_type,issueDate:r.issue_date,expiryDate:r.expiry_date,status:r.status,note:r.note,docPath:r.doc_path||""});
const fromP=(p)=>({vehicle:p.vehicle,doc_type:p.docType,issue_date:p.issueDate||"",expiry_date:p.expiryDate||"",status:p.status||"Valid",note:p.note||"",...(p.docPath!==undefined?{doc_path:p.docPath||""}:{})});
...
// with any group already on a vehicle so an existing group never disappears.
// Meter readings on fuel logs -- km on an odometer, hours on a generator's hour
// meter -- are NOT clean (Sept 2026, 3,259 vehicle logs):
//  - PLACEHOLDERS: staff type a repeated digit when they have no reading --
//    111111 (269x), 1111 (99x), 222222 (10x). Treated as no reading; naively the
//    fill after one shows +167,000 km and the one with it -166,000 km.
//  - NOT UPDATED: a reading equal to the last one (411 fills, mostly days
//    apart) means the meter wasn't read, not that the vehicle stood still.
//    "0 km" would be false, and would double the next fill's km/L.
//  - OUT-OF-SEQUENCE TYPOS: kept out by using the longest non-decreasing run of
//    real readings, which rejects a too-LOW and a too-HIGH typo alike.
const isPlaceholderReading=n=>{if(!n||n<=0)return true;const s=String(Math.round(n));return s.length>=4&&s.split("").every(c=>c===s[0]);};
// Returns Map(id -> {delta, litres, since, flag}). flag: "none" = no usable
// reading, "same" = not updated, "seq" = out of sequence. `litres` is the fuel
// added across the whole span (every fill since the last usable reading, this
// one included) so km/L stays right when a span covers several fills; `since`
// is the start date of such a span.
const meterDeltas=(logs)=>{
  const sorted=[...logs].sort((a,b)=>a.date.localeCompare(b.date)||(a.reading||0)-(b.reading||0)||String(a.id).localeCompare(String(b.id)));
  const real=sorted.filter(f=>!isPlaceholderReading(f.reading));
  const tails=[],tailAt=[],back=new Array(real.length).fill(-1);
  real.forEach((f,i)=>{let lo=0,hi=tails.length;while(lo<hi){const m=(lo+hi)>>1;if(tails[m]<=f.reading)lo=m+1;else hi=m;}tails[lo]=f.reading;tailAt[lo]=i;back[i]=lo>0?tailAt[lo-1]:-1;});
  const keep=new Set();for(let k=tails.length?tailAt[tails.length-1]:-1;k>=0;k=back[k])keep.add(real[k].id);
  const out=new Map();let last=null,lastIdx=-1,span=0;
  sorted.forEach((f,i)=>{
    span+=f.litres||0;
    if(isPlaceholderReading(f.reading)){out.set(f.id,{flag:"none"});return;}
    if(!keep.has(f.id)){out.set(f.id,{flag:"seq"});return;}
    if(last&&f.reading===last.reading){out.set(f.id,{flag:"same"});return;}
    out.set(f.id,last?{delta:f.reading-last.reading,litres:span,since:lastIdx===i-1?null:last.date}:{});
    last=f;lastIdx=i;span=0;
  });
  return out;
};
// A vehicle's usual km/L: the median over its measured fills, so one bad
// reading can't move it. Null until there are 5 fills to judge from.
const usualKmPerL=(deltas)=>{
  const r=[...deltas.values()].filter(d=>d.delta>0&&d.litres>0).map(d=>d.delta/d.litres).sort((a,b)=>a-b);
  return r.length>=5?r[Math.floor(r.length/2)]:null;
};
// Judge km/L against the vehicle's OWN usual, never a fleet-wide number: a
// King Long bus at 5 km/L is normal, a Hiace at 5 is not.
const kmPerLColor=(k,usual)=>!usual?"#161616":k<0.6*usual?"#DA1E28":k<0.8*usual?"#FF832B":"#24A148";
// Every fuel log's delta, computed over each asset's FULL history so a period
// report's first fill still measures from the fill before the period.
const allMeterDeltas=(fuelLogs)=>{
  const g={};(fuelLogs||[]).forEach(f=>{const k=(f.isGen?"g:":"v:")+f.asset;(g[k]=g[k]||[]).push(f);});
  const deltas=new Map(),usual=new Map();
...
const mergeVehicleGroups=(settings,vehicles)=>{
  ...
  const used=(vehicles||[]).map(v=>v.group).filter(Boolean);
  return [...new Set([...stored,...used].map(g=>String(g).trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
};
...
// Attachments. The `documents` bucket is PRIVATE, so the database stores the
// object PATH and a fresh 1-hour signed URL is minted each time someone opens
// a file -- a stored public URL would expose an invoice or a vehicle
...
function VehiclesPage({vehicles,setVehicles,locations,vehicleGroups,saveVehicleGroups,fuelLogs,workOrders,inspections,papers,svcReminders,canEdit,odoLog,setOdoLog}){
  const [sel,setSel]=useState(null);const [filt,setFilt]=useState("All");const [showAdd,setShowAdd]=useState(false);const [showSnap,setShowSnap]=useState(false);const [editV,setEditV]=useState(null);const [groupFilt,setGroupFilt]=useState("All");const [vQ,setVQ]=useState("");const [selIds,setSelIds]=useState([]);const [bulkGroup,setBulkGroup]=useState("");const [bulkSaving,setBulkSaving]=useState(false);const [showOdo,setShowOdo]=useState(false);const [odoVal,setOdoVal]=useState("");const [odoDate,setOdoDate]=useState(new Date().toISOString().split("T")[0]);
  ...
  const defForm={name:"",type:"Pickup",year:"2025",status:"Active",km:"0",driver:"",loc:locations[0]||"",nextSvc:"",plate:"",vin:"",group:""};
  const [form,setForm]=useState(defForm);
  const vQuery=vQ.trim().toLowerCase();
  const searched=vQuery?vehicles.filter(v=>[v.name,v.id,v.plate,v.vin,v.driver].some(x=>(x||"").toLowerCase().includes(vQuery))):vehicles;
  const list=searched.filter(v=>(filt==="All"||v.status===filt)&&(groupFilt==="All"||(groupFilt==="__none"?!v.group:v.group===groupFilt)));
  // Bulk group assignment. "Select all" ticks every vehicle currently listed, so
  // a status/group filter narrows exactly what gets assigned.
  const allSelected=list.length>0&&list.every(v=>selIds.includes(v.id));
  const toggleAll=()=>setSelIds(allSelected?[]:list.map(v=>v.id));
  const toggleOne=(id)=>setSelIds(prev=>prev.includes(id)?prev.filter(x=>x!==id):[...prev,id]);
  const onBulkPick=async(val)=>{
    if(val==="__new"){
      const name=(window.prompt("New group name")||"").trim();
      if(!name){setBulkGroup("");return;}
      if(saveVehicleGroups)await saveVehicleGroups([...(vehicleGroups||[]),name]);
      setBulkGroup(name);return;
    }
    setBulkGroup(val);
  };
  const applyBulkGroup=async()=>{
    if(!bulkGroup||!selIds.length)return;
    const grp=bulkGroup==="__none"?"":bulkGroup;
    setBulkSaving(true);
    try{
      for(const id of selIds){await db.updateVehicle(id,{grp});}
      setVehicles(vehicles.map(v=>selIds.includes(v.id)?{...v,group:grp}:v));
      setSelIds([]);setBulkGroup("");
  ...
  const startEdit=(v)=>{setForm({name:v.name,type:v.type,year:String(v.year),status:v.status,km:String(v.km||0),driver:v.driver||"",loc:v.loc||locations[0]||"",nextSvc:v.nextSvc||"",plate:v.plate||"",vin:v.vin||"",group:v.group||""});setEditV(v.id);setShowAdd(true);};
...
{odoHist.length>0&&<div style={{marginTop:14,background:"#fff",borderRadius:14,border:"1px solid #E8ECF1",padding:18}}><h4 style={{fontSize:13,fontWeight:700,marginBottom:10}}>Odometer History</h4><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:"#F4F4F4"}}>{["Date","Reading","Source"].map(h=>(<th key={h} style={th}>{h}</th>))}</tr></thead><tbody>{odoHist.map(o=>(<tr key={o.id}><td style={tc}>{o.date}</td><td style={{...tc,fontWeight:600}}>{o.reading.toLocaleString()} km</td><td style={tc}><span style={{fontSize:11,padding:"2px 8px",borderRadius:4,background:o.type==="fuel"?"#D0E2FF":"#F4F4F4",color:o.type==="fuel"?P:"#525252",fontWeight:600}}>{o.type==="fuel"?"Fuel Log":"Manual"}</span></td></tr>))}</tbody></table></div>}</div>);}
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{display:"flex",justifyContent:"space-between",gap:10,flexWrap:"wrap"}}><div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>{["All","Active","In Shop","Out of Service"].map(s=>(<button key={s} onClick={()=>{setFilt(s);setSelIds([]);}} style={{padding:"6px 14px",borderRadius:7,border:filt===s?`1.5px solid ${P}`:"1.5px solid #E0E0E0",background:filt===s?"#D0E2FF":"#fff",color:filt===s?P:"#525252",fontSize:12,fontWeight:600,cursor:"pointer"}}>{s} ({s==="All"?searched.length:searched.filter(v=>v.status===s).length})</button>))}<select value={groupFilt} onChange={e=>{setGroupFilt(e.target.value);setSelIds([]);}} style={{...inp,width:165,padding:"6px 10px",fontSize:12}}><option value="All">All groups</option>{(vehicleGroups||[]).map(g=>(<option key={g} value={g}>{g}</option>))}<option value="__none">(No group)</option></select><div style={{position:"relative",display:"flex",alignItems:"center"}}><Search size={14} color="#8D8D8D" style={{position:"absolute",left:10,pointerEvents:"none"}}/><input value={vQ} onChange={e=>{setVQ(e.target.value);setSelIds([]);}} placeholder="Search name, plate, VIN or driver..." style={{...inp,width:250,padding:"6px 28px 6px 30px",fontSize:12}}/>{vQ&&<button onClick={()=>{setVQ("");setSelIds([]);}} title="Clear search" style={{position:"absolute",right:6,background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",padding:2}}><X size={13} color="#8D8D8D"/></button>}</div></div>{canEdit&&<button onClick={()=>{setEditV(null);setForm(defForm);setShowAdd(true);}} style={{display:"flex",alignItems:"center",gap:5,padding:"8px 16px",borderRadius:9,background:P,color:"#fff",border:"none",fontSize:12,fontWeight:600,cursor:"pointer"}}><Plus size={15}/>Add Vehicle</button>}</div>
    {canEdit&&selIds.length>0&&(<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:10,background:"#EDF5FF",border:"1px solid #A6C8FF",flexWrap:"wrap"}}>
      <span style={{fontSize:13,fontWeight:700,color:"#0043CE"}}>{selIds.length} vehicle{selIds.length>1?"s":""} selected</span>
      <select value={bulkGroup} onChange={e=>onBulkPick(e.target.value)} style={{...inp,width:200,padding:"7px 10px",fontSize:12}}>
        <option value="">Assign to group...</option>
        {(vehicleGroups||[]).map(g=>(<option key={g} value={g}>{g}</option>))}
        <option value="__none">Remove from group</option>
        <option value="__new">+ New group...</option>
      </select>
      <button onClick={applyBulkGroup} disabled={!bulkGroup||bulkSaving} style={{padding:"8px 18px",borderRadius:8,border:"none",background:(bulkGroup&&!bulkSaving)?P:"#C6C6C6",color:"#fff",fontSize:12,fontWeight:700,cursor:(bulkGroup&&!bulkSaving)?"pointer":"not-allowed"}}>{bulkSaving?"Applying...":"Apply"}</button>
      <button onClick={()=>{setSelIds([]);setBulkGroup("");}} style={{padding:"8px 14px",borderRadius:8,border:"1.5px solid #E0E0E0",background:"#fff",color:"#525252",fontSize:12,fontWeight:600,cursor:"pointer"}}>Clear</button>
    </div>)}
    <div style={{background:"#fff",borderRadius:14,border:"1px solid #E8ECF1",overflow:"hidden"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:"#F4F4F4"}}>{canEdit&&<th style={{...th,width:36}}><input type="checkbox" title="Select all" checked={allSelected} onChange={toggleAll} style={{cursor:"pointer"}}/></th>}{["Vehicle","Type","Group","Status","Driver","Mileage","Cost/km"].map(h=>(<th key={h} style={th}>{h}</th>))}{canEdit&&<th style={th}>Actions</th>}</tr></thead><tbody>{list.length===0&&(<tr><td colSpan={canEdit?9:7} style={{...tc,textAlign:"center",color:"#8D8D8D",padding:"26px 12px"}}>{vQuery?<>No vehicles matching "{vQ}".</>:"No vehicles match these filters."}</td></tr>)}{list.map(v=>(<tr key={v.id} style={{cursor:"pointer",background:selIds.includes(v.id)?"#F8FAFF":""}}>{canEdit&&<td style={tc}><input type="checkbox" checked={selIds.includes(v.id)} onChange={()=>toggleOne(v.id)} style={{cursor:"pointer"}}/></td>}<td style={tc} onClick={()=>setSel(v.id)}><div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontSize:18}}>{v.img}</span><div><div style={{fontWeight:600}}>{v.name}</div><div style={{fontSize:11,color:"#8D8D8D"}}>{v.id}</div></div></div></td><td style={tc} onClick={()=>setSel(v.id)}>{v.type}</td><td style={tc} onClick={()=>setSel(v.id)}>{v.group?<span style={{fontSize:11,fontWeight:600,padding:"3px 9px",borderRadius:20,background:"#EDF5FF",color:"#0043CE"}}>{v.group}</span>:<span style={{color:"#C6C6C6"}}>-</span>}</td><td style={tc} onClick={()=>setSel(v.id)}><Badge label={v.status}/></td><td style={tc} onClick={()=>setSel(v.id)}>{v.driver||"Unassigned"}</td><td style={tc} onClick={()=>setSel(v.id)}>{(v.km||0).toLocaleString()} km</td><td style={tc} onClick={()=>setSel(v.id)}>{(()=>{const c=calcCostKm(v.id);return c>0?fmt(c):"-";})()}</td>{canEdit&&<td style={tc}><div style={{display:"flex",gap:4}}><button onClick={(e)=>{e.stopPropagation();startEdit(v);}} style={{padding:"4px 8px",borderRadius:5,border:"1px solid #E0E0E0",background:"#fff",cursor:"pointer"}}><Pencil size={12} color="#525252"/></button><button onClick={(e)=>{e.stopPropagation();handleDelete(v.id);}} style={{padding:"4px 8px",borderRadius:5,border:"1px solid #E0E0E0",background:"#fff",cursor:"pointer"}}><Trash2 size={12} color="#DA1E28"/></button></div></td>}</tr>))}</tbody></table></div>
    {showAdd&&(<Modal title={editV?"Edit Vehicle":"Add New Vehicle"} onClose={()=>{setShowAdd(false);setEditV(null);}}><Field label="Vehicle Name *"><input style={inp} placeholder="e.g. Toyota Hilux GD-6" value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/></Field><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Field label="Type"><select style={inp} value={form.type} onChange={e=>setForm({...form,type:e.target.value})}><option>Pickup</option><option>Van</option><option>Box Truck</option><option>Light Truck</option><option>Heavy Truck</option><option>Semi Truck</option><option>Tipper</option></select></Field><Field label="Year"><input style={inp} type="number" value={form.year} onChange={e=>setForm({...form,year:e.target.value})}/></Field></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Field label="Status"><select style={inp} value={form.status} onChange={e=>setForm({...form,status:e.target.value})}><option>Active</option><option>In Shop</option><option>Out of Service</option></select></Field><Field label="Mileage (km)"><input style={inp} type="number" value={form.km} onChange={e=>setForm({...form,km:e.target.value})}/></Field></div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Field label="Plate"><input style={inp} placeholder="LAG-123-AB" value={form.plate} onChange={e=>setForm({...form,plate:e.target.value})}/></Field><Field label="VIN/SN"><input style={inp} placeholder="Vehicle ID number" value={form.vin||""} onChange={e=>setForm({...form,vin:e.target.value})}/></Field></div><Field label="Driver"><input style={inp} placeholder="Driver name" value={form.driver} onChange={e=>setForm({...form,driver:e.target.value})}/></Field><Field label="Group"><select style={inp} value={form.group||""} onChange={e=>setForm({...form,group:e.target.value})}><option value="">-- No group --</option>{(vehicleGroups||[]).map(g=>(<option key={g} value={g}>{g}</option>))}</select></Field><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><Field label="Location"><select style={inp} value={form.loc} onChange={e=>setForm({...form,loc:e.target.value})}><option value="">-- Select Location --</option>{locations.map(l=>(<option key={l}>{l}</option>))}</select></Field><Field label="Next Service"><input style={inp} type="date" value={form.nextSvc} onChange={e=>setForm({...form,nextSvc:e.target.value})}/></Field></div><div style={{display:"flex",gap:10,justifyContent:"flex-end",marginTop:8}}><button onClick={()=>{setShowAdd(false);setEditV(null);}} style={{padding:"9px 20px",borderRadius:8,border:"1.5px solid #E0E0E0",background:"#fff",color:"#525252",fontSize:13,fontWeight:600,cursor:"pointer"}}>Cancel</button><button onClick={handleSave} disabled={!form.name} style={{display:"flex",alignItems:"center",gap:6,padding:"9px 20px",borderRadius:8,border:"none",background:form.name?P:"#C6C6C6",color:"#fff",fontSize:13,fontWeight:600,cursor:form.name?"pointer":"not-allowed"}}><Save size={14}/>{editV?"Update":"Save"}</button></div></Modal>)}
  </div>);
}
...
function WOPage({workOrders,setWorkOrders,vehicles,generators,vendors,canEdit,docUploads}){
  const [filt,setFilt]=useState("All");const [showAdd,setShowAdd]=useState(false);const [editW,setEditW]=useState(null);const [woQ,setWoQ]=useState("");
  const defForm={asset:"",type:"Repair",priority:"Medium",desc:"",assignee:"",due:"",cost:"",isGen:false,invoicePath:""};
  const [form,setForm]=useState(defForm);
  const woQuery=woQ.trim().toLowerCase();
  // Vehicle name, WO id and vendor. `assignee` holds the vendor name.
  const searched=woQuery?workOrders.filter(w=>[w.asset,w.id,w.assignee].some(x=>(x||"").toLowerCase().includes(woQuery))):workOrders;
  const list=filt==="All"?searched:searched.filter(w=>w.status===filt);
  const handleSave=async()=>{try{if(editW){const up=fromWO({...form,id:editW,cost:parseFloat(form.cost)||0,invoicePath:docUploads?(form.invoicePath||""):undefined});await db.updateWorkOrder(editW,up);setWorkOrders(workOrders.map(w=>w.id===editW?{...w,...form,cost:parseFloat(form.cost)||0}:w));setEditW(null);}else{const nid=`WO-${Date.now().toString(36).toUpperCase()}`;const nw=fromWO({id:nid,...form,status:"Open",cost:parseFloat(form.cost)||0,invoicePath:docUploads?(form.invoicePath||""):undefined});const saved=await db.addWorkOrder(nw);if(saved){setWorkOrders(prev=>[...prev,toWO(saved)]);}else{const all=await db.getWorkOrders();setWorkOrders(all.map(toWO));}}setShowAdd(false);setForm(defForm);}catch(e){alert("Error: "+e.message);}};
...
function FuelPage({fuelLogs,setFuelLogs,vehicles,generators,canEdit,odoLog,setOdoLog,vendors,docUploads}){
  ...
  const defForm={asset:"",date:new Date().toISOString().split("T")[0],litres:"",cost:"",reading:"",station:"",fuelType:"Diesel",costPerL:"",receiptPath:""};
  const [form,setForm]=useState(defForm);
  ...
  const {deltas:fDeltas,usual:fUsual}=useMemo(()=>allMeterDeltas(fuelLogs),[fuelLogs]);
  const calcConsumption=(f)=>{const d=fDeltas.get(f.id)||{};const grey="#8D8D8D";if(d.flag==="none")return {text:"no reading",color:grey,title:"No usable meter reading on this entry (blank, or a placeholder like 111111)"};if(d.flag==="same")return {text:"not updated",color:grey,title:"Same reading as the previous fill - the meter was not read"};if(d.flag==="seq")return {text:"check reading",color:"#DA1E28",title:"Out of sequence with the readings around it - probably a typo"};if(!(d.delta>0&&d.litres>0))return {text:"-",color:grey,title:""};const span=d.since?" - measured over every fill since "+d.since:"";if(f.isGen){const r=d.litres/d.delta;return {text:r.toFixed(1),color:r>15?"#DA1E28":r>8?"#FF832B":"#24A148",title:(Math.round(d.delta*10)/10).toLocaleString()+" hrs on "+Math.round(d.litres)+" L"+span};}const k=d.delta/d.litres,u=fUsual.get(f.asset);return {text:k.toFixed(1),color:kmPerLColor(k,u),title:d.delta.toLocaleString()+" km on "+Math.round(d.litres)+" L"+(u?" - usual for this vehicle "+u.toFixed(1)+" km/L":"")+span};};
  const getLastReading=(assetId)=>{const logs=fuelLogs.filter(f=>f.asset===assetId&&!isPlaceholderReading(f.reading)).sort((a,b)=>b.date.localeCompare(a.date)||b.reading-a.reading);return logs.length>0?logs[0].reading:null;};
  const handleSave=async()=>{const reading=parseFloat(form.reading)||0;const entry={asset:form.asset,date:form.date,litres:parseFloat(form.litres)||0,cost:parseFloat(form.cost)||0,reading,station:form.station,isGen:tab==="generators",fuelType:form.fuelType||"Diesel",receiptPath:docUploads?(form.receiptPath||""):undefined};try{if(editF){await db.updateFuelLog(editF,fromFL(entry));setFuelLogs(fuelLogs.map(f=>f.id===editF?{...f,...entry}:f));setEditF(null);}else{const saved=await db.addFuelLog(fromFL(entry));if(saved){setFuelLogs(prev=>[...prev,toFL(saved)]);}else{const all=await db.getFuelLogs();setFuelLogs(all.map(toFL));}if(reading>0){const odoSaved=await db.addOdoLog({asset:form.asset,reading,date:form.date,type:"fuel"});setOdoLog([...odoLog,toOdo(odoSaved)]);}}setShowAdd(false);setForm(defForm);}catch(e){alert("Error: "+e.message);}};
  ...
  const startEdit=(f)=>{const cpl=f.litres>0?(f.cost/f.litres).toFixed(2):"";setForm({asset:f.asset,date:f.date,litres:String(f.litres),cost:String(f.cost),reading:String(f.reading||""),station:f.station||"",fuelType:f.fuelType||"Diesel",costPerL:cpl,receiptPath:f.receiptPath||""});setEditF(f.id);setShowAdd(true);};
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{background:"#fff",borderRadius:14,border:"1px solid #E8ECF1",overflow:"hidden"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:"#F4F4F4"}}>{["Date","Asset",tab==="vehicles"?"Odometer":"Hours","Litres","Cost",tab==="vehicles"?"km/L":"L/hr","Station",...(docUploads?["Receipt"]:[]),canEdit?"":""].filter(Boolean).map(h=>(<th key={h} style={th}>{h}</th>))}</tr></thead><tbody>{data.length===0?(<tr><td colSpan={(canEdit?8:7)+(docUploads?1:0)} style={{padding:20,textAlign:"center",color:"#8D8D8D",fontSize:13}}>No {tab} fuel logs yet.</td></tr>):data.map((f,idx)=>{const cons=calcConsumption(f);return(<tr key={f.id}><td style={tc}>{f.date}</td><td style={{...tc,fontWeight:600}}>{getAssetName(f.asset,f.isGen)}</td><td style={tc}>{(f.reading||0).toLocaleString()} {f.isGen?"hrs":"km"}</td><td style={tc}>{f.litres.toLocaleString()} L</td><td style={{...tc,fontWeight:600}}>{fmt(f.cost)}</td><td style={{...tc,fontWeight:700,color:cons.color}} title={cons.title}>{cons.text}</td><td style={{...tc,color:"#8D8D8D"}}>{f.station}</td>{docUploads&&<td style={tc}><DocLink path={f.receiptPath} title="View receipt"/></td>}{canEdit&&<td style={tc}><div style={{display:"flex",gap:4}}><button onClick={()=>startEdit(f)} style={{padding:"4px 8px",borderRadius:5,border:"1px solid #E0E0E0",background:"#fff",cursor:"pointer"}}><Pencil size={12} color="#525252"/></button><button onClick={()=>handleDelete(f.id)} style={{padding:"4px 8px",borderRadius:5,border:"1px solid #E0E0E0",background:"#fff",cursor:"pointer"}}><Trash2 size={12} color="#DA1E28"/></button></div></td>}</tr>);})}</tbody></table></div>
...
function PapersPage({vehicles,papers,setPapers,canEdit,docTypes,setDocTypes,docUploads}){
  ...
  const defForm={vehicle:"",docType:docTypes[0]||"",issueDate:"",expiryDate:"",status:"Valid",note:"",docPath:""};
  const [form,setForm]=useState(defForm);
  ...
  const handleSave=async()=>{const entry={...form,status:autoStatus(form.expiryDate),docPath:docUploads?(form.docPath||""):undefined};try{if(editP){await db.updatePaper(editP,fromP(entry));setPapers(papers.map(p=>p.id===editP?{...p,...entry}:p));setEditP(null);}else{const saved=await db.addPaper(fromP(entry));if(saved){setPapers(prev=>[...prev,toP(saved)]);}else{const all=await db.getPapers();setPapers(all.map(toP));}}setShowAdd(false);setForm(defForm);}catch(e){alert("Error: "+e.message);}};
  ...
  const startEdit=(p)=>{setForm({vehicle:p.vehicle,docType:p.docType,issueDate:p.issueDate||"",expiryDate:p.expiryDate||"",status:p.status,note:p.note||"",docPath:p.docPath||""});setEditP(p.id);setShowAdd(true);};
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{background:"#fff",borderRadius:14,border:"1px solid #E8ECF1",overflow:"hidden"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:"#F4F4F4"}}>{["Vehicle","Document","Issue Date","Expiry Date","Status","Note",...(docUploads?["File"]:[]),canEdit?"":""].filter(Boolean).map(h=>(<th key={h} style={th}>{h}</th>))}</tr></thead><tbody>{list.length===0?<tr><td colSpan={(canEdit?7:6)+(docUploads?1:0)} style={{padding:20,textAlign:"center",color:"#8D8D8D",fontSize:13}}>No documents found</td></tr>:list.map(p=>(<tr key={p.id}><td style={{...tc,fontWeight:600}}>{getVName(p.vehicle)}</td><td style={tc}>{p.docType}</td><td style={tc}>{p.issueDate||"-"}</td><td style={{...tc,fontWeight:600}}>{p.expiryDate||"-"}</td><td style={tc}><Badge label={p.status}/></td><td style={{...tc,color:"#8D8D8D",fontSize:12}}>{p.note||"-"}</td>{docUploads&&<td style={tc}><DocLink path={p.docPath} title="View document"/></td>}{canEdit&&<td style={tc}><div style={{display:"flex",gap:4}}><button onClick={()=>startEdit(p)} style={{padding:"4px 8px",borderRadius:5,border:"1px solid #E0E0E0",background:"#fff",cursor:"pointer"}}><Pencil size={12} color="#525252"/></button><button onClick={()=>handleDelete(p.id)} style={{padding:"4px 8px",borderRadius:5,border:"1px solid #E0E0E0",background:"#fff",cursor:"pointer"}}><Trash2 size={12} color="#DA1E28"/></button></div></td>}</tr>))}</tbody></table></div>
...
function ReportsPage({vehicles:allVehicles,generators:allGenerators,drivers,workOrders,fuelLogs,dieselReadings,dieselPurchases,dieselDistributions,vehicleGroups}){
  ...
  // -- which is also why the scope only applies on the tabs that SHOW the Group
  // control: the diesel tabs are generator-based, and silently emptying them
  // from a control the user cannot see would just look like missing data.
  const GROUPABLE_REPORTS=["fleet","fuel","maintenance","driver"];
  const groupActive=groupFilter!=="All"&&GROUPABLE_REPORTS.includes(report);
  const vehicles=!groupActive?allVehicles:allVehicles.filter(v=>(groupFilter==="__none"?!v.group:v.group===groupFilter));
  const generators=groupActive?[]:allGenerators;
  const gvIds=new Set(vehicles.map(v=>v.id));
  ...
  // same table no matter what range was picked, plus a phantom extra "Open".
  const inRange=(d)=>!d?false:(d>=dateFrom&&d<=dateTo);
...
function SettingsPage({locations,setLocations,vehicleGroups,saveVehicleGroups,vendorTypes,setVendorTypes,users,setUsers,user,setUser,appSettings,setAppSettings,dieselLocks,setDieselLocks}){
  ...
  const [uf,setUf]=useState({name:"",email:"",password:"",role:"Viewer",store_location:""});const [newLoc,setNewLoc]=useState("");const [newGrp,setNewGrp]=useState("");const [showAddGrp,setShowAddGrp]=useState(false);const [newVT,setNewVT]=useState("");const [showAddVT,setShowAddVT]=useState(false);const [loading,setLoading]=useState(false);const [msg,setMsg]=useState("");
  ...
  const handleAddGroup=async()=>{const n=newGrp.trim();if(!n)return;try{await saveVehicleGroups([...(vehicleGroups||[]),n]);setNewGrp("");setShowAddGrp(false);}catch(e){alert("Error: "+e.message);}};
  const handleRemoveGroup=async(g)=>{if(!confirm("Remove the group '"+g+"' from the list? Vehicles already tagged with it keep the tag, and the group reappears here until you reassign them on the Vehicles page."))return;try{await saveVehicleGroups((vehicleGroups||[]).filter(x=>x!==g));}catch(e){alert("Error: "+e.message);}};
  ...
  const handleStoreChange=async(uid,store)=>{try{await db.updateProfile(uid,{store_location:store||null});setUsers(users.map(u=>u.id===uid?{...u,store_location:store||null}:u));}catch(e){alert("Error: "+e.message);}};
  ...
  const tabs=user?.role==="Store Staff"?["profile"]:isAdmin?["profile","users","locations","vehicle groups","vendor types","diesel"]:["profile","users","locations","vehicle groups","vendor types"];
  return(<div style={{maxWidth:800}}><div style={{display:"flex",gap:8,marginBottom:20,flexWrap:"wrap"}}>{tabs.map(t=>(<button key={t} onClick={()=>setTab(t)} style={{padding:"8px 20px",borderRadius:8,border:tab===t?`1.5px solid ${P}`:"1.5px solid #E0E0E0",background:tab===t?"#D0E2FF":"#fff",color:tab===t?P:"#525252",fontSize:13,fontWeight:600,cursor:"pointer",textTransform:"capitalize"}}>{t}</button>))}</div>
    ...
    {tab==="users"&&(<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><h3 style={{fontSize:15,fontWeight:700,margin:0}}>Team Members</h3>{isSA&&<button onClick={()=>setShowAddUser(true)} style={{display:"flex",alignItems:"center",gap:5,padding:"8px 16px",borderRadius:9,background:P,color:"#fff",border:"none",fontSize:12,fontWeight:600,cursor:"pointer"}}><Plus size={15}/>Add User</button>}</div>
      <div style={{background:"#fff",borderRadius:14,border:"1px solid #E8ECF1",overflow:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",minWidth:isSA?720:420}}><thead><tr style={{background:"#F4F4F4"}}>{["Name","Email","Role",isSA?"Location":"",isSA?"Actions":""].filter(Boolean).map(h=>(<th key={h} style={th}>{h}</th>))}</tr></thead><tbody>{users.map(u=>(<tr key={u.id}><td style={{...tc,fontWeight:600}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:32,height:32,borderRadius:"50%",background:"#D0E2FF",display:"flex",alignItems:"center",justifyContent:"center",fontSize:12,fontWeight:700,color:P}}>{u.avatar||"?"}</div>{u.name}</div></td><td style={tc}>{u.email}</td><td style={tc}><Badge label={u.role}/></td>{isSA&&<td style={tc}>{u.role==="Store Staff"?<select style={{...inp,width:150,padding:"5px 8px",fontSize:11,...((!u.store_location)?{borderColor:"#DA1E28"}:{})}} value={u.store_location||""} onChange={e=>handleStoreChange(u.id,e.target.value)}><option value="">-- pick store --</option>{locations.map(l=>(<option key={l} value={l}>{l}</option>))}</select>:<span style={{fontSize:11,color:"#8D8D8D"}}>All stores</span>}</td>}{isSA&&<td style={tc}><select style={{...inp,width:140,padding:"5px 8px",fontSize:11}} value={u.role} onChange={e=>handleRoleChange(u.id,e.target.value)}><option>Super Admin</option><option>Fleet Manager</option><option>Store Staff</option><option>Viewer</option></select></td>}</tr>))}</tbody></table></div>
    ...
    {tab==="vehicle groups"&&(<div><div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}><h3 style={{fontSize:15,fontWeight:700,margin:0}}>Vehicle Groups</h3><button onClick={()=>setShowAddGrp(true)} style={{display:"flex",alignItems:"center",gap:5,padding:"8px 16px",borderRadius:9,background:P,color:"#fff",border:"none",fontSize:12,fontWeight:600,cursor:"pointer"}}><Plus size={15}/>Add Group</button></div>
      <div style={{fontSize:12,color:"#8D8D8D",marginBottom:14}}>Tag vehicles on the Vehicles page (tick several and use "Assign to group"), then filter Reports by group.</div>
      <div style={{display:"flex",flexWrap:"wrap",gap:8}}>{(vehicleGroups||[]).map(g=>(<div key={g} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 14px",background:"#fff",borderRadius:8,border:"1px solid #E8ECF1"}}><span style={{fontSize:13,fontWeight:600}}>{g}</span><button onClick={()=>handleRemoveGroup(g)} style={{background:"none",border:"none",color:"#DA1E28",fontSize:11,cursor:"pointer",fontWeight:600}}>Remove</button></div>))}</div>
      {(vehicleGroups||[]).length===0&&<div style={{padding:20,textAlign:"center",color:"#8D8D8D",fontSize:13}}>No groups yet. Add one to start tagging vehicles.</div>}
      {showAddGrp&&(<Modal title="Add Vehicle Group" onClose={()=>setShowAddGrp(false)}><Field label="Group Name *"><input style={inp} placeholder="e.g. Operations" value={newGrp} onChange={e=>setNewGrp(e.target.value)}/></Field><div style={{display:"flex",gap:10,justifyContent:"flex-end"}}><button onClick={()=>setShowAddGrp(false)} style={{padding:"9px 20px",borderRadius:8,border:"1.5px solid #E0E0E0",background:"#fff",color:"#525252",fontSize:13,fontWeight:600,cursor:"pointer"}}>Cancel</button><button onClick={handleAddGroup} disabled={!newGrp.trim()} style={{padding:"9px 20px",borderRadius:8,border:"none",background:newGrp.trim()?P:"#C6C6C6",color:"#fff",fontSize:13,fontWeight:600,cursor:newGrp.trim()?"pointer":"not-allowed"}}>Add</button></div></Modal>)}
    </div>)}
...
function FleetProAppInner(){
  ...
  useEffect(()=>{let live=true;(async()=>{
    ...
    if(live)setDocUploads({fuel,wo,papers});
  })();return()=>{live=false;};},[]);
  const vehicleGroups=mergeVehicleGroups(appSettings,vehicles);
  const saveVehicleGroups=async(list)=>{
    const clean=[...new Set((list||[]).map(g=>String(g).trim()).filter(Boolean))].sort((a,b)=>a.localeCompare(b));
    setAppSettings(prev=>({...prev,vehicle_groups:clean}));
    try{await db.setAppSetting("vehicle_groups",clean,user?.uid);}catch(e){console.error("saveVehicleGroups:",e);}
    return clean;
  };
  const isStoreStaff=user?.role==="Store Staff";
  ...
  return(<div style={{minHeight:"100vh",background:"#F7F8FC",fontFamily:"'DM Sans',system-ui,sans-serif"}}><link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700&display=swap" rel="stylesheet"/>
    ...
    <main style={{marginLeft:sw,padding:mob?"14px 10px":"20px 24px",transition:"margin-left 0.2s",minHeight:"calc(100vh - 56px)"}}>
      <Routes>
        ...
        <Route path="/fuel" element={<FuelPage docUploads={docUploads.fuel} fuelLogs={fuelLogs} setFuelLogs={setFuelLogs} vehicles={vehicles} generators={generators} canEdit={canEdit} odoLog={odoLog} setOdoLog={setOdoLog} vendors={vendors}/>}/>
        <Route path="/workorders" element={<WOPage docUploads={docUploads.wo} workOrders={workOrders} setWorkOrders={setWorkOrders} vehicles={vehicles} generators={generators} vendors={vendors} canEdit={canEdit}/>}/>
        <Route path="/papers" element={<PapersPage docUploads={docUploads.papers} vehicles={vehicles} papers={papers} setPapers={setPapers} canEdit={canEdit} docTypes={docTypes} setDocTypes={setDocTypes}/>}/>
