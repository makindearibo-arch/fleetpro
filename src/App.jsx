...
// Distance covered between fuel entries, from the odometer readings. Two traps
// in the real data (Sept 2026, 3,259 vehicle fuel logs):
//  - PLACEHOLDERS: staff type a repeated digit when they have no reading --
//    111111 (269x), 1111 (99x), 222222 (10x). Treated as "no reading", or the
...
//    real readings, which rejects a too-LOW and a too-HIGH typo alike (an extra
//    digit would otherwise make every later fill look like it went backwards).
// Returns Map(id -> {km, since, flag}). flag: "none" = no usable reading,
// "seq" = out of sequence. `since` = date of the reading the distance was
// measured from, set only when that is not the immediately previous fill.
const odoDistances=(logs)=>{
  const sorted=[...logs].sort((a,b)=>a.date.localeCompare(b.date)||(a.reading||0)-(b.reading||0)||String(a.id).localeCompare(String(b.id)));
  const isPlaceholder=n=>{if(!n||n<=0)return true;const s=String(Math.round(n));return s.length>=4&&s.split("").every(c=>c===s[0]);};
  const real=sorted.filter(f=>!isPlaceholder(f.reading));
  ...
  const out=new Map();let last=null,lastIdx=-1;
  sorted.forEach((f,i)=>{
    if(isPlaceholder(f.reading)){out.set(f.id,{flag:"none"});return;}
    if(!keep.has(f.id)){out.set(f.id,{flag:"seq"});return;}
    out.set(f.id,last?{km:f.reading-last.reading,since:lastIdx===i-1?null:last.date}:{});
    last=f;lastIdx=i;
  });
  return out;
};
