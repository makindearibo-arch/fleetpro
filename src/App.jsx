...
function WOPage({workOrders,setWorkOrders,vehicles,generators,vendors,canEdit}){
  ...
  const searched=woQuery?workOrders.filter(w=>(w.asset||"").toLowerCase().includes(woQuery)):workOrders;
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    {list.length===0&&(<div style={{background:"#fff",borderRadius:12,border:"1px solid #E8ECF1",padding:"28px 18px",textAlign:"center",color:"#8D8D8D",fontSize:13}}>{woQuery?<>No work orders for a vehicle matching "{woQ}".</>:"No work orders yet."}</div>)}
