...
function VehiclesPage({vehicles,setVehicles,locations,vehicleGroups,saveVehicleGroups,fuelLogs,workOrders,inspections,papers,svcReminders,canEdit,odoLog,setOdoLog}){
  const [sel,setSel]=useState(null);const [filt,setFilt]=useState("All");const [showAdd,setShowAdd]=useState(false);const [showSnap,setShowSnap]=useState(false);const [editV,setEditV]=useState(null);const [groupFilt,setGroupFilt]=useState("All");const [selIds,setSelIds]=useState([]);const [bulkGroup,setBulkGroup]=useState("");const [bulkSaving,setBulkSaving]=useState(false);const [showOdo,setShowOdo]=useState(false);const [odoVal,setOdoVal]=useState("");const [odoDate,setOdoDate]=useState(new Date().toISOString().split("T")[0]);
  ...
  const list=vehicles.filter(v=>(filt==="All"||v.status===filt)&&(groupFilt==="All"||(groupFilt==="__none"?!v.group:v.group===groupFilt)));
